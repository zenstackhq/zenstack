...
`;
    ...
    describe('Nested routes', () => {
        ...
        it('returns nested self-links in JSON:API responses for all nested operations', async () => {
            ...
            // POST /user/u1/post — nested create
            const created = await nestedHandler({
                method: 'post',
                path: '/user/u1/post',
                client: nestedClient,
            ...
            expect(created.body.links.self).toBe(`http://localhost/api/user/u1/post/${postId}`);
            expect(created.body.data.links.self).toBe(`http://localhost/api/user/u1/post/${postId}`);

            // GET /user/u1/post — nested collection read
            const collection = await nestedHandler({
                method: 'get',
                path: '/user/u1/post',
                client: nestedClient,
            });
            expect(collection.status).toBe(200);
            expect(collection.body.links.self).toBe('http://localhost/api/user/u1/post');
            expect(collection.body.data[0].links.self).toBe(`http://localhost/api/user/u1/post/${postId}`);

            // GET /user/u1/post/:id — nested single read
            ...
            expect(single.body.links.self).toBe(`http://localhost/api/user/u1/post/${postId}`);
            expect(single.body.data.links.self).toBe(`http://localhost/api/user/u1/post/${postId}`);

            // PATCH /user/u1/post/:id — nested update
            ...
            expect(updated.body.links.self).toBe(`http://localhost/api/user/u1/post/${postId}`);
            expect(updated.body.data.links.self).toBe(`http://localhost/api/user/u1/post/${postId}`);
        });
